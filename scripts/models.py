...
# How does this correspond to 1h on 30 threads?
...
def get_ILP_local(w, k, sigma=2, nSolutions=1, seed=None, sketch_size=1, method=-1):
    ...
    try:
        ...
        x = {
            node: [
                m.addVar(vtype=GRB.BINARY, name=f"x_({node}, {j})") for j in range(w)
            ]
            for node in nodes
        }
...
def get_ILP_fwd(w, k, sigma=2, nSolutions=1, heuristics=0.1, seed=None, method=-1):
    ...
    try:
        ...
        # Create variables
        x = {
            node: m.addVar(lb=0, ub=w - 1, vtype=GRB.INTEGER, name=f"x_{node}")
            for node in nodes
        }
        ...
        if k % w == 1:
            for x in range(n + 1, n + 2):
                ...
                for neck, rots in necklaces.items():
                    p = len(rots)
                    neck_edges = [
                        (rots[i][:n], rots[(i + 1) % p][:n]) for i in range(p)
                    ]
